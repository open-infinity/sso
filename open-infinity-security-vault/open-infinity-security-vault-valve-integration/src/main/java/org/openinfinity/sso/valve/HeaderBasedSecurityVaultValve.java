...
import org.openinfinity.sso.valve.mapper.RequestToHeaderMapper;
import org.openinfinity.sso.valve.mapper.RequestToIdentityMapper;

/**
 * Security vault intercepts every request and looks for a username from the request. 
 ...
 * 
 * Notice that HTTP headers are modifiable through user session. Therefore identity provider / service provider needs to take care of the possible header manipulation attempts.
 *  
 *  
 ...
 * @since 1.0.0
 */
public class HeaderBasedSecurityVaultValve extends ValveBase {

	/**
	 * Logger for this class.
	 */
	private static final Logger LOGGER = Logger.getLogger(HeaderBasedSecurityVaultValve.class.getName());
	...
	static {
		if (SESSION_IDENTIFIER == null) {
			PropertiesUtil.init();
			SESSION_IDENTIFIER = PropertiesUtil.loadValue(GlobalVariables.HEADER_BASED_SESSION_KEY);
		}
	...
	public void invoke(Request request, Response response) throws IOException, ServletException {
		String sessionId = (String) request.getHeader(SESSION_IDENTIFIER);
		...
		if (sessionId != null && IdentityContext.loadIdentity(sessionId) == null) {
			...
			RequestToIdentityMapper requestToHeaderMapper = new RequestToHeaderMapper();
			Identity identity = requestToHeaderMapper.map(request);
